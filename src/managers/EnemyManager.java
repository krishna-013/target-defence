...
//		tempMethod();
	...
	private void loadRoadDirArr() {
		roadDirArr = Utils.GetRoadDirectionArr(playing.getGame().getTileManager().getTypeArr(), start, end);
	}

//	private void tempMethod() {
//		int[][] arr = Utils.GetRoadDirectionArr(playing.getGame().getTileManager().getTypeArr(), start, end);
...
//		PathPoint currTile = new PathPoint((int) (e.getX() / 32), (int) (e.getY() / 32));
	...
	private boolean isTilesSame(PathPoint currTile, PathPoint newTile) {
	...
	public int getCountOfLiveEnemies() {
		int size = 0;
		for (Enemy e : enemies)
			if (e.isAlive())
				size++;
